...
const ProjectConversationsTab: React.FC<ProjectConversationsTabProps> = ({
  ...
  return <div className="space-y-4">
      ...
      {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        </div> : conversations.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {conversations.map(conversation => <ConversationCard key={conversation.id} conversation={conversation} linkState={{
        fromShared: isSharedProject
      }} />)}
